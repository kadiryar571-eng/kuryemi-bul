...
| **Login/Register** | `index.html?auth=login` (modal) | Auth entry: email/password + Google OAuth | No |
...
| **Security** | `guvenlik.html` | Hazır değil — içeriksiz yer tutucu | No |
...
**Treatment:** Redirect to `index.html?auth=login&next=…`, which opens the auth modal. `runSessionGuard()` handles this automatically for protected pages.
**Recovery:** Re-login flow.

### 7.5 Permission Errors
**Treatment:** Page-level auth check. If `!KB.isAuthed()` on protected page → `KB.gotoAuth('login')`.
